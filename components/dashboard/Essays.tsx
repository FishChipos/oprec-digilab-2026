...
const Essays = ({ data }: { data: data | null }) => {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			if (!userId) {
				setMessage({
					text: "You must be logged in to save essays",
					type: "error",
				});
				return;
			}
			...
			setMessage({
				text: "Failed to save essays. Please try again.",
				type: "error",
			});
		} finally {
			setLoading(false);
	...
	const handleSubmitApplication = async (e: React.FormEvent) => {
		...
		try {
			...
			if (!userId) {
				setMessage({
					text: "You must be logged in to submit application",
					type: "error",
				});
				return;
			}
			...
			if (error instanceof Error && error.message.includes("NULL")) {
				setMessage({
					text: "Failed to submit application. There are still some empty fields.",
					type: "error",
				});
			} else {
				setMessage({
		...
		<div className="space-y-6 p-4 max-w-3xl mx-auto">
			<div className="space-y-2">
				...
				<p className="text-gray-500 dark:text-gray-400">
					Mohon tulis jawaban yang lengkap untuk pertanyaan-pertanyaan berikut.
					Setiap esai minimal harus 50 kata.
				</p>
			</div>
				...
				<form onSubmit={handleSubmit} className="space-y-8">
					<div className="space-y-2">
						<Label htmlFor="motivation" className="text-lg font-medium">
							Apa alasan atau motivasi utama kamu ingin bergabung sebagai
							Asisten Lab Digital?
						...
						{wordCounts.motivation > 0 && wordCounts.motivation < 50 && (
							<p className="text-amber-500 text-sm">
								Mohon tulis minimal 50 kata ({50 - wordCounts.motivation} kata
								lagi).
							</p>
						)}
					</div>
					...
					<div className="space-y-2">
						...
						{wordCounts.experience > 0 && wordCounts.experience < 50 && (
							<p className="text-amber-500 text-sm">
								Mohon tulis minimal 50 kata ({50 - wordCounts.experience} kata
								lagi).
							</p>
						)}
					</div>
					...
					<div className="space-y-2">
						...
						{wordCounts.contribution > 0 && wordCounts.contribution < 50 && (
							<p className="text-amber-500 text-sm">
								Mohon tulis minimal 50 kata ({50 - wordCounts.contribution} kata
								lagi).
							</p>
						)}
					</div>
					...
					<div className="space-y-2">
						<Label htmlFor="commitment" className="text-lg font-medium">
							Bagaimana cara untuk mengatur waktu dan komitmen kamu jika
							terpilih menjadi Asisten Lab?
							<span className="ml-2 text-sm text-gray-500">
						...
						{wordCounts.commitment > 0 && wordCounts.commitment < 50 && (
							<p className="text-amber-500 text-sm">
								Mohon tulis minimal 50 kata ({50 - wordCounts.commitment} kata
								lagi).
							</p>
						)}
					</div>
					...
					<div className="flex justify-between">
						...
						<Button
							...
							disabled={loading || isSubmitted || isDeadlinePassed}
						>
